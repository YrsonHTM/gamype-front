...
export class NominasService {
  ...
  generatePdfNomina(data: Nomina, fileName: string, Empleado: any, periodoNomina): void {
    const doc = new jsPDF();
  ...
  <!-- Sección de información dividida -->
  <div style="display: flex; justify-content: space-between; margin: 15px 0;">
    <!-- Información de la empresa (izquierda) -->
    <div style="width: 48%;">
      <h4 style="color: black; font-size: 14px; margin: 5px 0;">Empresa: "${this.companyService.getCompanValue().companyName}"</h4>
      <h5 style="color: black; font-size: 12px; margin: 5px 0;">Dirección: "${this.companyService.getCompanValue().addres}"</h5>
      <h5 style="color: black; font-size: 12px; margin: 5px 0;">Teléfono: "${this.companyService.getCompanValue().contactNumber}"</h5>
      <h5 style="color: black; font-size: 12px; margin: 5px 0;">Correo: "${this.companyService.getCompanValue().email}"</h5>
    </div>
    
    <!-- Información del empleado (derecha) -->
    <div style="width: 48%; text-align: right;">
      <h4 style="color: black; font-size: 14px; margin: 5px 0;">Nómina de: "${Empleado?.nombresApellidos}"</h4>
      <h5 style="color: black; font-size: 12px; margin: 5px 0;">Identificador: "${Empleado?.identificadorEmpresa}"</h5>
      <h5 style="color: black; font-size: 12px; margin: 5px 0;">Numero documento: "${Empleado?.tipoNroDocumento}"</h5>
      <h5 style="color: black; font-size: 10px; margin: 5px 0;">Periodo: "${periodoNomina}"</h5>
    </div>
  </div>
  <h5 style="color: black; font-size: 10px; margin: 5px 0;">Dias: ${data.diasTrabajados}</h5>
  <!-- Detalles de la nómina (sin cambios) -->
  ...
  <footer style="display: flex; flex-direction: row-reverse; margin-top: 20px">${formatFecha}</footer>
    ...
    html2canvas(element, { scale: 3 }).then(canvas => {
      ...
      // Guardar el PDF
      doc.save(`${Empleado.nombresApellidos}.pdf`);
